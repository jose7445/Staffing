...
export default defineEventHandler(async (event) => {
    if(config.envType === "DEV") console.log("PUT DELETE /api/quizzes/[id]/questions-categories");
    ...
    },{ "$pull": { "questionsCategories": {"_id": id} } }, {new:true});
        if (!responseData) {
            ...
            if(config.envType === "DEV") console.log("Delete quizz question category");
            return responseData;
        }
    } catch (err) {
